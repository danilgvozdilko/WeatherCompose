...
import com.example.weathercompose.model.WeatherItem
import com.example.weathercompose.navigation.WeatherScreens
import com.example.weathercompose.utils.formatDate
import com.example.weathercompose.utils.formatDecimals
import com.example.weathercompose.widgets.*
...
fun MainScreen(
    ...
    val curCity: String = if (city!!.isBlank()) "Kyiv" else city
    val weatherData = produceState<DataOrException<Weather, Boolean, Exception>>(
        initialValue = DataOrException(loading = true)
    ) {
        value = mainViewModel.getWeatherData(curCity, "imperial")
    }.value
...
fun MainContent(data: Weather) {
    ...
    Column(
        ...
        HumidityWindPressureRow(weather = data.list[0], true)
        Divider()
